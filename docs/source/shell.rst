...
RPCs are commands that can be sent to the node to query it, control it and manage it. RPCs don't typically
do anything that changes the global ledger, but they may change node-specific data in the database. Each RPC
is one method on the ``CordaRPCOps`` interface, and may return a stream of events that will be shown on screen
...
The same syntax is also used to specify the parameters for RPCs.
...
* There's no convenient way to express an ``Amount<Issued<Currency>>`` in text, so APIs and flows that need that are hard to invoke.
* SSH access is currently not available.
* There is no cross-server command history or command completion.
