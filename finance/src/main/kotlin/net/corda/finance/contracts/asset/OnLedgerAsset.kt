...
abstract class OnLedgerAsset<T : Any, C : CommandData, S : FungibleAsset<T>> : Contract {
    companion object {
         ...
         * @param tx A builder, which may contain inputs, outputs and commands already. The relevant components needed
         *           to move the cash will be added on top.
         * @param payments A list of (payee, amount) pairs that will be satisfied atomically in the transaction.
         * @param acceptableStates a list of acceptable input states to use.
         * @param deriveState a function to derive an output state based on an input state, amount for the output
         * and public key to pay to.
        ...
        fun <S : FungibleAsset<T>, T: Any> generateSpend(tx: TransactionBuilder,
            ...
            val remainingFromEachIssuer = statesGroupedByIssuer
                    .mapValues {
                        it.value.map { it.state.data.amount }.sumOrThrow()
            ...
            // Just copy a key across as the change key. This works but leaks private data (it reveals which outputs
            // are the change to observers). In bitcoinj we derive a fresh key here and then shuffle the outputs
            // to ensure it's hard to follow value flows through the transaction graph.
            //
            // TODO: Select a fresh key for the change outputs (integrate with confidential identities).
            ...
            // TODO: Shuffle the output states.

            for (state in gathered) tx.addInputState(state)
            for (state in outputStates) tx.addOutputState(state)

            // TODO: What if we already have a move command with the right keys? Filter it out here or in platform code?
