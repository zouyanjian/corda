...
abstract class ReceiveMultiple(val requests: List<ReceiveRequest<SessionData>>) : WaitingRequest {
    @Transient
    override val stackTraceInCaseOfProblems: StackSnapshot = StackSnapshot()

    abstract fun isComplete(received: LinkedHashMap<FlowSessionInternal, Pair<ReceiveRequest<SessionData>, ReceivedSessionMessage<*>>>): Boolean
    abstract fun shouldResumeIfRelevant(message: ExistingSessionMessage, session: FlowSessionInternal): Boolean
...
class ReceiveAll(requests: List<ReceiveRequest<SessionData>>) : ReceiveMultiple(requests) {
    override fun isComplete(received: LinkedHashMap<FlowSessionInternal, Pair<ReceiveRequest<SessionData>, ReceivedSessionMessage<*>>>) = received.keys == requests.map { it.session }.toSet()

    override fun shouldResumeIfRelevant(message: ExistingSessionMessage, session: FlowSessionInternal): Boolean = requests.all { it.session.receivedMessages.map { it.message }.any { it is SessionData || it is SessionEnd } }
}
